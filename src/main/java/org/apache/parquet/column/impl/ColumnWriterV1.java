...
public final class ColumnWriterV1 implements ColumnWriter {
    private static final Log LOG = Log.getLog(org.apache.parquet.column.impl.ColumnWriterV1.class);
    ...
    public static ThreadLocal<ColumnDescriptor> currentColumn = new ThreadLocal<>();

    public void flush() {
        try {
            currentColumn.set(this.path);
            if (valueCount > 0) {
                writePage();
            }
