...
public class ParquetFileReader implements Closeable {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(org.apache.parquet.hadoop.ParquetFileReader.class);
    ...
    /**
     * @deprecated use {@link org.apache.parquet.hadoop.ParquetFileReader#readFooter(Configuration, FileStatus, MetadataFilter)}
     */
    @Deprecated
    ...
    public static org.apache.parquet.hadoop.ParquetFileReader open(Configuration conf, Path file) throws IOException {
        return new org.apache.parquet.hadoop.ParquetFileReader(conf, file);
    }

    public static org.apache.parquet.hadoop.ParquetFileReader open(Configuration conf, Path file, MetadataFilter filter) throws IOException {
        return new org.apache.parquet.hadoop.ParquetFileReader(conf, file, filter);
    }

    public static org.apache.parquet.hadoop.ParquetFileReader open(Configuration conf, Path file, ParquetMetadata footer) throws IOException {
        return new org.apache.parquet.hadoop.ParquetFileReader(conf, file, footer);
    }
    ...
    private org.apache.parquet.hadoop.ColumnChunkPageReadStore currentRowGroup = null;
    ...
    public PageReadStore readNextRowGroup() throws IOException {
        this.currentRowGroup = readRowGroup(currentBlock, null);
        advanceToNextBlock();
        return currentRowGroup;
    }

    public PageReadStore readNextRowGroup(ColumnChunkMetaData mc) throws IOException {
        this.currentRowGroup = readRowGroup(currentBlock, Arrays.asList(mc));
        advanceToNextBlock();
        return currentRowGroup;
    }

    public PageReadStore readNextRowGroup(ColumnChunkMetaData[] mcs) throws IOException {
        this.currentRowGroup = readRowGroup(currentBlock, Arrays.asList(mcs));
        advanceToNextBlock();
        return currentRowGroup;
    }
     ...
     * @throws IOException if an error occurs while reading
     */
    public org.apache.parquet.hadoop.ColumnChunkPageReadStore readRowGroup(int blockIndex, Iterable<ColumnChunkMetaData> mcs) throws IOException {
        ...
        org.apache.parquet.hadoop.ColumnChunkPageReadStore rowGroup = new org.apache.parquet.hadoop.ColumnChunkPageReadStore(block.getRowCount());
        ...
        if (null == mcs) {
            mcs = block.getColumns();
        }
        for (ColumnChunkMetaData mc : mcs) {
        ...
        for (ConsecutiveChunkList consecutiveChunks : allChunks) {
            ...
            for (Chunk chunk : chunks) {
                try {
                    ...
                    throw new RuntimeException(chunk.descriptor.col.toString(), e);
                }
            }
        }
