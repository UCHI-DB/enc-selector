...
import edu.uchicago.cs.encsel.dataset.feature.report.SelTimeUsage
...
object RunFeatureOnMainColumn extends App {
  ...
  val columns = persist.em.createQuery("SELECT c FROM Column c WHERE c.parentWrapper IS NULL", classOf[ColumnWrapper])
